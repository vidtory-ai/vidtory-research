#!/usr/bin/env python3
"""Show ready and blocked $imagegen jobs for a character sheet run."""
...
def input_image_view(item: object, run_dir: Path) -> dict[str, object]:
    if not isinstance(item, dict) or not isinstance(item.get("path"), str):
        return {"path": None, "role": None, "exists": False}
...
def job_view(job: dict[str, object], run_dir: Path, completed: set[str]) -> dict[str, object]:
    ...
    return {
        ...
        "input_images": [input_image_view(item, run_dir) for item in inputs],
